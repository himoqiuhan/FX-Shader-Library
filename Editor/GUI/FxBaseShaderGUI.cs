...
{
    public class FxBaseShaderGUI : UnityEditor.ShaderGUI
    {
        #region GUIVariables
        ...
        // Main Visual
        private MaterialProperty polarCoordinatesParamsProp { get; set; }
        private MaterialProperty generalColorProp { get; set; }
        
        protected MaterialProperty useMainRChannelAsAlphaProp { get; set; }
        ...
        protected MaterialProperty inverseDepthFadeProp { get; set; }
        ...
        public virtual void FindProperties(MaterialProperty[] properties)
        {
            ...
            polarCoordinatesParamsProp = FindProperty("_PolarCoordinatesParams", properties, false);
            generalColorProp = FindProperty("_Color", properties, false);
            useMainRChannelAsAlphaProp = FindProperty("_UseMainRChannelAsAlpha", properties, false);
            ...
            inverseDepthFadeProp = FindProperty("_InverseDepthFade", properties, false);
        ...
        protected virtual void DrawGeneralSection(Material material)
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(generalSection.anim.faded))
            {
                ...
                generalUVMode = material.IsKeywordEnabled("_USE_WORLD_PROJECTION_UV") ? UVMode.WorldProjection : generalUVMode;
                bool usePolarCoordinates = material.IsKeywordEnabled("_USE_POLAR_COORDINATES");
                var polarCoordinatesParamsPropValue = polarCoordinatesParamsProp.vectorValue;
                var polarCoordinatesCenter = new Vector2(polarCoordinatesParamsPropValue.x, polarCoordinatesParamsPropValue.y);
                ...
                usePolarCoordinates = EditorGUILayout.Toggle("转换为极坐标", usePolarCoordinates);
                if (usePolarCoordinates)
                {
                    polarCoordinatesCenter = EditorGUILayout.Vector2Field("极坐标中心", polarCoordinatesCenter);
                    polarCoordinatesParamsPropValue.z = EditorGUILayout.FloatField("半径缩放", polarCoordinatesParamsPropValue.z);
                    polarCoordinatesParamsPropValue.w = EditorGUILayout.FloatField("角度偏移", polarCoordinatesParamsPropValue.w);
                }
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    material.DisableKeyword("_USE_WORLD_PROJECTION_UV");
                    if (generalUVMode == UVMode.ScreenSpace)
                    {
                        material.EnableKeyword("_USE_SCREEN_SPACE_UV");
                    }
                    else if (generalUVMode == UVMode.WorldProjection)
                    {
                        material.EnableKeyword("_USE_WORLD_PROJECTION_UV");
                    }

                    if (usePolarCoordinates)
                    {
                        material.EnableKeyword("_USE_POLAR_COORDINATES");
                    }
                    else
                    {
                        material.DisableKeyword("_USE_POLAR_COORDINATES");
                    }

                    polarCoordinatesParamsPropValue.x = polarCoordinatesCenter.x;
                    polarCoordinatesParamsPropValue.y = polarCoordinatesCenter.y;
                    polarCoordinatesParamsProp.vectorValue = polarCoordinatesParamsPropValue;
        ...
        protected virtual void DrawMainVisualSection(Material material)
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(mainVisualSection.anim.faded))
            {
                ...
                materialEditor.ShaderProperty(useMainRChannelAsAlphaProp, "R通道作为Alpha");
        ...
        protected virtual void DrawSoftParticleSection(Material material)
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(softParticleSection.anim.faded))
            {
                ...
                bool inverseDepthFade = inverseDepthFadeProp.floatValue > 0.5f;
                if (isSoftParticle)
                {
                    
                    inverseDepthFade = EditorGUILayout.Toggle("反向软粒子", inverseDepthFade);
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    if (isSoftParticle)
                    {
                        ...
                        if (inverseDepthFade)
                        {
                            inverseDepthFadeProp.floatValue = 1;
                        }
                        else
                        {
                            inverseDepthFadeProp.floatValue = 0;
                        }
                    }
                    else
                    {
